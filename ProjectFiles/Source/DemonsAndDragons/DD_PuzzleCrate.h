...
private:
	...
	UFUNCTION()
	void BeginOverlapWithOther(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex,
		bool bFromSweep, const FHitResult& SweepResult);

	UFUNCTION()
	void EndOverlapWithOther(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex);

	UFUNCTION()
	int GetOverlappingWeightObjectCount();

	/*
	* Pressure plate reference pointer
	*/
	UPROPERTY()
	class ADD_PressurePlate* PlateRef;
};